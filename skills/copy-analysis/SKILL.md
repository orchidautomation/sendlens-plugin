...
- Use `rendered_outbound_context` to verify how copy rendered in practice.
...
- If personalization quality depends on campaign-specific variables, inspect those values through `lead_payload_kv` for that campaign rather than assuming shared payload columns.
