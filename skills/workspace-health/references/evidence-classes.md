...
| `hydrated_reply_body` | `reply_body_text`/`reply_content_preview` returned after `prepare_campaign_analysis` or `fetch_reply_text`, or rows with `reply_email_id` from `reply_email_context`/`reply_context`. | "Fetched inbound reply body..." "The fetched reply says..." | Only use for fetched statuses/pages. Do not imply unfetched replies contain the same language. |
