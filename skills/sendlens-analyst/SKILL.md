...
5. **Load one campaign for depth.** Call `load_campaign_data` before deep copy, reply, ICP, launch, or campaign work. Call `prepare_campaign_analysis` when fetched reply wording could change working/not-working, scale/kill, copy, ICP, or client-safe recommendations. After every preparation call, use `reply_coverage_summary` to report the aggregate unique human reply count separately from selected-status hydration, including OOO exclusion, `latest_of_thread`, per-status fetched/hydrated counts, exhaustion, the numeric gap, and the neutral scope explanation.
