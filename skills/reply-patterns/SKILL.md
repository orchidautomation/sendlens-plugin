...
- Query `reply_context` first. If the user needs actual reply wording, run `hydrate_reply_text` for exactly one campaign in the default `sync_newest` mode, then query `reply_context` again.
...
- Use default `hydrate_reply_text(mode="sync_newest")` when checking current replies; use `mode="continue"` only when the user explicitly wants older rows beyond the saved cursor. Avoid `mode="auto"` when the user expects fresh reply bodies.
