...
- `reply_coverage_summary` reports `aggregate_reply_count`, `hydrated_reply_count`, `fetched_reply_count`, `coverage_gap_count`, `coverage_scope`, per-status counts, and selected-bucket exhaustion; the scope records selected statuses, OOO exclusion, and `latest_of_thread=true`
