...
- Before listing campaigns, tables, metrics, or recommendations, read `active_data_state` from `setup_doctor` or `workspace_snapshot`. If it says `no_workspace`, `demo_workspace`, or `cached_workspace_refresh_disabled`, make that the first sentence of the answer and do not let cache size or table counts imply live provider data.
...
- `active_data_state`: When present in `workspace_snapshot` or `setup_doctor`, treat it as the source of truth for whether data is unavailable, synthetic demo fixtures, an existing cache without live refresh credentials, or a refresh-configured local workspace. Lead with this state before inventory or analysis.
