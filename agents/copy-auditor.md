...
7. If personalization depends on campaign variables, inspect values through `lead_payload_kv` for that campaign instead of assuming shared payload columns. Use `campaign-metadata-coverage` to distinguish scalar, sparse, and non-scalar inputs when the exact payload key is not already named.
