...
type NormalizedStepAnalyticsRow = {
  ...
  uniqueRepliesAutomatic: unknown;
  uniqueReplies: unknown;
  clicks: unknown;
  uniqueClicks: unknown;
  bounces: unknown;
  opportunities: unknown;
  uniqueOpportunities: unknown;
...
function recentActivityWindow(asOf = new Date(), timezone = "UTC"): RecentActivityWindow {
  const resolvedTimezone = isValidTimeZone(timezone) ? timezone : "UTC";
  const localDate = calendarDateInTimeZone(asOf, resolvedTimezone);
...
export const recentActivityWindowForTests = recentActivityWindow;
...
function buildRecentActivitySelection(
  ...
  windowByCampaign: Map<string, RecentActivityWindow>,
  evaluatedAt: string,
  coverageByCampaign: Map<string, RecentActivityCoverage>,
  ...
  let availableCampaigns = 0;

  for (const campaign of campaigns) {
    const campaignId = String(campaign.id ?? "");
    if (!campaignId) continue;
    const timezone = campaignRecentTimezone(campaign);
    const window = windowByCampaign.get(campaignId) ?? recentActivityWindow(new Date(), timezone.timezone);
    windowKeys.add(`${window.startDate}:${window.endDate}`);
    const analyticsRow = recentAnalyticsByCampaign.get(campaignId);
    const requestedCoverage = coverageByCampaign.get(campaignId) ?? "unavailable";
    const recentSentCount = requestedCoverage === "available"
      ? recentAnalyticsSentCount(analyticsRow)
      : null;
    const campaignCoverage: RecentActivityCoverage = requestedCoverage === "available"
    ...
    if (campaignCoverage === "available") availableCampaigns += 1;
  ...
  const windows = Array.from(windowKeys).map((key) => {
  ...
  return {
    ...
    windows,
    coverage: unavailableCampaigns > 0 && availableCampaigns === 0 ? "unavailable" : "available",
...
function firstRecipientEmail(value: unknown, fallback: unknown) {
  ...
  for (const candidate of candidates) {
    const email = emailOrNull(candidate);
    if (email) return email;
  }
  return emailOrNull(fallback) ?? "";
}
...
async function storeWorkspaceAccounts(
  ...
  for (const account of accounts) {
    ...
    accountRows.push(`(
      ...
      ${sqlFloat(account.stat_warmup_score ?? account.warmup_score ?? warm.health_score)},
      ${sqlString(account.provider_code ?? account.provider)},
    ...
    campaigns
      ...
      .map((campaign) => {
        ...
        return `(
          ...
          ${sqlBool(campaign.first_email_text_only)},
          ${sqlBool(campaign.open_tracking)},
          ${sqlBool(campaign.link_tracking)},
          ${sqlBool(campaign.stop_on_reply)},
          ${sqlBool(campaign.stop_on_auto_reply)},
          ${sqlBool(campaign.match_lead_esp)},
          ${sqlBool(campaign.allow_risky_contacts)},
          ${sqlBool(campaign.disable_bounce_protect)},
          ${sqlBool(campaign.insert_unsubscribe_header)},
...
async function fetchReplyEmails(
  ...
  return emails
    ...
    .map((email): ReplyEmailRecord => ({
      ...
      toEmail: firstRecipientEmail(email.to_address_email_list, email.lead),
...
function emailRecordFromInstantly(
  ...
  const toEmail = firstRecipientEmail(email.to_address_email_list, email.lead);
  return {
    id,
    campaignId,
    threadId: String(email.thread_id ?? ""),
...
async function fetchLeadSample(
  ...
  if (fullRaw) {
    const coverage = await instantly.listAllLeadsWithCoverage(apiKey, campaignId, 50, {
      limit: DEFAULT_PAGE_SIZE,
    });
    for (const lead of coverage.items) {
      collectLead(lead);
    }
  ...
  let pagesFetched = 0;
  let terminationReason = "max_pages";
...
async function hydrateCampaignRefreshBundle(
  ...
  if (skippedStepAnalytics > 0) {
    console.warn(
      `[sendlens] Skipped ${skippedStepAnalytics} malformed step analytics row(s) while refreshing a campaign.`,
    );
  }
  ...
  const requestedFullRaw = options.forceHybrid
    ? false
  ...
  return {
    ...
    outboundSample,
    totalUniqueReplies,
    fullRaw: leadSample.source === "full_raw",
    detailElapsedMs,
    stepElapsedMs,
    dailyElapsedMs,
    leadSampleElapsedMs,
...
async function storeCampaignData(
  ...
  await insertRows(
    ...
    [
      `(
        ...
        ${sqlString(mapCampaignStatus(campaign.status))},
        ${sqlInt(campaign.daily_limit)},
        ${sqlBool(detail.text_only ?? campaign.text_only)},
        ${sqlBool(detail.first_email_text_only ?? campaign.first_email_text_only)},
        ${sqlBool(detail.open_tracking ?? campaign.open_tracking)},
        ${sqlBool(detail.link_tracking ?? campaign.link_tracking)},
        ${sqlBool(detail.stop_on_reply ?? campaign.stop_on_reply)},
        ${sqlBool(detail.stop_on_auto_reply ?? campaign.stop_on_auto_reply)},
        ${sqlBool(detail.match_lead_esp ?? campaign.match_lead_esp)},
        ${sqlBool(detail.allow_risky_contacts ?? campaign.allow_risky_contacts)},
        ${sqlBool(detail.disable_bounce_protect ?? campaign.disable_bounce_protect)},
        ${sqlBool(detail.insert_unsubscribe_header ?? campaign.insert_unsubscribe_header)},
    ...
    await insertRows(
      ...
      [
        `(
          ...
          ${sqlInt(analytics.reply_count_unique)},
          ${sqlInt(analytics.reply_count_automatic)},
          ${sqlInt(analytics.reply_count_automatic_unique)},
          ${sqlInt(analytics.link_click_count)},
          ${sqlInt(analytics.link_click_count_unique)},
          ${sqlJson(analytics.open_count_unique_by_step)},
          ${sqlJson(analytics.reply_count_unique_by_step)},
          ${sqlJson(analytics.reply_count_automatic_unique_by_step)},
          ${sqlJson(analytics.link_click_count_unique_by_step)},
          ${sqlInt(analytics.bounced_count)},
          ${sqlInt(analytics.unsubscribed_count)},
          ${sqlInt(analytics.completed_count)},
          ${sqlInt(analytics.total_opportunities)},
  ...
  await insertRows(
    ...
    normalizeStepAnalyticsRows(stepAnalytics).validRows.map(
      (row) => `(
        ...
        ${sqlInt(row.uniqueRepliesAutomatic)},
        ${sqlInt(row.uniqueReplies)},
        ${sqlInt(row.clicks)},
        ${sqlInt(row.uniqueClicks)},
        ${sqlInt(row.bounces)},
        ${sqlInt(row.opportunities)},
        ${sqlInt(row.uniqueOpportunities)},
        ${sqlInt(row.meetingsBooked)},
        CURRENT_TIMESTAMP
      )`,
    ),
  );
  ...
  const totalLeads = Number(analyticsRow.leads_count ?? 0) || 0;
  const totalSent = Number(analyticsRow.emails_sent_count ?? 0) || 0;
  const ingestMode = leadSample.source === "full_raw" ? "full" : "hybrid";
  const note =
    ingestMode === "full"
...
async function refreshWorkspaceAtomicallyWithProviderMode(options: RefreshOptions = {}) {
  ...
  try {
    // Full workspace refreshes rebuild from an empty shadow database so
    // resources deleted or deactivated upstream cannot survive locally.
    // Only an explicitly scoped campaign refresh patches the last good cache.
    if (options.campaignIds?.length && await shouldSeedShadowFromLive(liveDbPath)) {
...
export async function refreshWorkspace(options: RefreshOptions = {}) {
  ...
  return withCacheProviderMode(providerMode.mode, async () => {
    ...
    if (providerMode.mode === "all") {
      ...
      const scopedMisses: ProviderRefreshOutcome[] = [];
      ...
      const smartleadOptions = refreshOptionsForProvider(options, "smartlead");
      const effectiveProviderScope: SourceProvider[] = [
        ...(instantlyOptions ? (["instantly"] as const) : []),
        ...(smartleadOptions ? (["smartlead"] as const) : []),
      ];
      const refreshedProviders: ProviderRefreshOutcome[] = [];
      const allRefreshStartedAt = new Date().toISOString();
      const source = options.source ?? (options.campaignIds?.length ? "manual" : "session_start");
      const persistAggregateFailure = async (failedProvider: ProviderRefreshOutcome) => {
        const failedProviders = [...scopedMisses, failedProvider];
        const endedAt = summaries[summaries.length - 1]?.last_refreshed_at ??
          new Date().toISOString();
        const aggregateRefreshScope = buildRefreshScope({
          provider: "all",
          campaignIds: options.campaignIds,
          failedScopedLookup: failedProviders.some((failure) =>
            failure.refreshScope.type === "failed_scoped_lookup"
          ),
        });
        ...
        await writeRefreshStatus({
          status: "failed",
          source,
          lastRefreshScope: aggregateRefreshScope.type,
          ...
          message: refreshedProviders.length > 0
            ? `All-provider refresh failed after ${refreshedProviders.map((row) => row.provider).join(", ")} completed; ${failedProvider.provider} failed: ${failedProvider.message}`
            : `All-provider refresh failed for ${failedProvider.provider}: ${failedProvider.message}`,
        });
      };
        ...
        try {
          ...
          const failedProvider = {
            ...
            refreshScope: buildRefreshScope({
              provider: "instantly",
              campaignIds: instantlyOptions.campaignIds,
              failedScopedLookup: Boolean(instantlyOptions.campaignIds?.length) &&
          ...
          if (
            ...
            await persistAggregateFailure(failedProvider);
            throw error;
          }
          scopedMisses.push(failedProvider);
        }
      }
        ...
        try {
          ...
          const failedProvider = {
            ...
            refreshScope: buildRefreshScope({
              provider: "smartlead",
              campaignIds: smartleadOptions.campaignIds,
              failedScopedLookup: Boolean(smartleadOptions.campaignIds?.length) &&
                isScopedCampaignMiss(error),
            }),
            message: error instanceof Error ? error.message : String(error),
          };
      ...
      const partialFailures = scopedMisses.map((miss) => ({
        ...
        refreshScope: miss.refreshScope,
      ...
      let aggregateSummary = summaries[summaries.length - 1];
      if (aggregateWorkspaceId) {
        const aggregateDb = await getDb();
        try {
          aggregateSummary = {
            ...(await buildWorkspaceSummary(
              aggregateDb,
              aggregateWorkspaceId,
              "all",
              { liveRefreshReady: refreshedProviders.length > 0 },
            )),
            refresh_certificate: refreshCertificate,
          };
        } finally {
...
async function refreshInstantlyWorkspace(options: RefreshOptions = {}) {
  ...
  try {
    ...
    if (scopedRefresh) {
      ...
      const recentAnalyticsByCampaign = new Map<string, Record<string, unknown>>();
      const windowByCampaign = new Map<string, RecentActivityWindow>();
      const coverageByCampaign = new Map<string, RecentActivityCoverage>();
      const campaignsByWindow = new Map<string, {
      ...
      for (const campaign of campaigns) {
        ...
        if (!campaignId) continue;
        const timezone = campaignRecentTimezone(campaign);
        const window = recentActivityWindow(asOf, timezone.timezone);
        const key = `${window.startDate}:${window.endDate}`;
        windowByCampaign.set(campaignId, window);
        const group = campaignsByWindow.get(key) ?? {
          window,
          campaignIds: [],
        };
        group.campaignIds.push(campaignId);
        campaignsByWindow.set(key, group);
      }

      if (campaignsByWindow.size > 0) {
        for (const group of campaignsByWindow.values()) {
          try {
            const recentAnalytics = await instantly.getCampaignAnalytics(apiKey, {
              campaignIds: group.campaignIds,
              startDate: group.window.startDate,
              endDate: group.window.endDate,
            });
            for (const campaignId of group.campaignIds) {
              coverageByCampaign.set(campaignId, "available");
            }
            for (const row of recentAnalytics) {
              recentAnalyticsByCampaign.set(String(row.campaign_id ?? ""), row);
            }
          } catch (error) {
            for (const campaignId of group.campaignIds) {
              coverageByCampaign.set(campaignId, "unavailable");
            }
            await appendTraceLog("refresh.recent_activity_unavailable", {
              provider: SOURCE_PROVIDER,
              window: group.window,
              campaignCount: group.campaignIds.length,
    ...
    await appendTraceLog("refresh.campaign_scope", {
      ...
      recentActivityWindows: recentActivitySelection.windows,
    ...
    const analytics = analyticsCampaignIds.length > 0
      ? await instantly.getCampaignAnalytics(apiKey, {
    ...
    await clearWorkspaceData(
      db,
      workspaceId,
      options.campaignIds?.length ? selectedCampaignIds : undefined,
    );
    await storeCampaignDirectory(
      db,
      workspaceId,
    ...
    const refreshCertificate = buildRefreshCertificate({
      requestedProviderScope: "instantly",
      effectiveProviderScope: ["instantly"],
      refreshedProviders: [{
        provider: "instantly",
    ...
    const refreshMetadata = {
      ...
      recent_activity_windows: recentActivitySelection.windows,
    ...
    const summary = {
      ...(await buildWorkspaceSummary(db, workspaceId, "instantly", {
