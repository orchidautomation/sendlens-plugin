...
async function seedLeadsAndReplies(db: Awaited<ReturnType<typeof getDb>>) {
  ...
  await run(
    ...
    `INSERT OR REPLACE INTO sendlens.reply_emails
     ...
     VALUES
     ('${DEMO_WORKSPACE_ID}', 'reply-sl-alpha-1', '${DEMO_CAMPAIGN_SMARTLEAD_ID}', 'thread-sl-alpha-1', 'alex.rivera@example.invalid', 'sl-lead-1001', '<demo-sl-alpha-1@example.invalid>', 'sender-smartlead@demo.invalid', 'alex.rivera@example.invalid', 'sender-smartlead@demo.invalid', 'Re: Operational capacity at Northstar Health', 'We are comparing referral handoff options now. The Smartlead demo thread is relevant.', '<p>We are comparing referral handoff options now.</p>', CURRENT_TIMESTAMP, false, 0.88, 1, 'We are comparing referral handoff options now...', 'inbound', '0', '0', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
  );
  await run(
    db,
